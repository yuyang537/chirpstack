 ...
 * 模块概述
 * ========
 * 该模块是ChirpStack LoRaWAN网络服务器的配置文件生成工具，用于创建标准格式的配置文件模板。
 * 在ChirpStack系统中，配置文件是定义服务器行为、连接参数和系统设置的核心组件。
 * 该模块通过模板引擎生成详细注释的配置文件，帮助用户理解和配置ChirpStack的各个方面。
 *
 * 文件功能
 * ========
 * 该文件实现了一个命令行工具，用于生成ChirpStack的默认配置文件模板。
 * 它使用Handlebars模板引擎，基于系统默认配置生成带有详细注释的TOML格式配置文件。
 * 生成的配置文件包含所有可配置选项，并附有说明和默认值，便于用户理解和修改。
 *
 * 主要组件
 * ========
 * - run(): 主函数，生成配置文件模板
 * - 配置模板：定义了配置文件的结构和内容
 * - 默认值注入：将系统默认配置值注入到模板中
 *
 * 关键流程
 * ========
 * 1. 定义配置文件的模板字符串，包含所有配置部分
 * 2. 从系统默认配置中获取默认值
 * 3. 使用Handlebars模板引擎将默认值注入到模板中
 * 4. 将生成的配置文件输出到标准输出
 *
 * 注意事项
 * ========
 * - 生成的配置文件是一个起点，用户应根据实际需求修改
 * - 某些配置选项可能依赖于特定的编译特性
 * - 配置文件中的敏感信息（如数据库密码）应妥善保护
 * - 修改配置后需要重启服务器才能生效
 * - 建议在修改配置前备份原配置文件
 */
