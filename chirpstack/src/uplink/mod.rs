 ...
 * 
 * # 模块概述
 * 本模块是ChirpStack系统的核心组件之一，负责处理来自LoRaWAN终端设备的上行数据。
 * 它实现了LoRaWAN协议的上行通信部分，包括数据帧的解析、去重、处理和路由。
 * 
 * # 文件功能
 * - 定义上行数据处理的核心结构和流程
 * - 实现上行数据的去重和聚合
 * - 处理不同类型的上行消息（数据、入网请求等）
 * - 管理网关元数据和过滤
 * - 提供上行数据的监控和统计
 * 
 * # 主要组件
 * - UplinkFrameSet：表示一组来自不同网关的相同上行帧
 * - RelayContext：处理中继设备的上行数据
 * - 子模块：
 *   - data：处理数据上行消息
 *   - join：处理入网请求消息
 *   - stats：处理网关统计信息
