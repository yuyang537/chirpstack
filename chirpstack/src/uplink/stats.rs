 ...
 * 
 * # 模块概述
 * 本模块负责处理LoRaWAN网关发送的统计信息(Stats)。在LoRaWAN网络中，网关会定期向网络服务器
 * 发送状态统计信息，包括网关的运行状态、信号质量、数据包统计等。这些信息对于监控网络健康状况、
 * 诊断问题和优化网络性能至关重要。本模块实现了网关统计信息的接收、解析、存储和处理功能。
 * 
 * # 文件功能
 * - 接收和解析网关发送的统计信息
 * - 更新网关的状态和最后活动时间
 * - 存储网关统计数据用于历史分析
 * - 计算和记录网关的占空比(Duty Cycle)统计
 * - 根据需要更新网关配置
 * - 支持不同区域和调制方式的数据速率映射
 * 
 * # 主要组件
 * - Stats结构体：处理网关统计信息的主要结构，包含处理过程中的所有状态和数据
 * - handle方法：处理网关统计信息的入口点
 * - 辅助方法：用于更新网关状态、保存统计数据、更新配置等
 * - per_modultation_to_per_dr函数：将调制方式统计转换为数据速率统计
 * 
 * # 关键流程
 * - 网关统计处理流程：
 *   1. 接收网关发送的统计信息
