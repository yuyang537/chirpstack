 ...
 * 模块概述
 * ========
 * 设备模块是ChirpStack LoRaWAN网络服务器的核心组件之一，负责管理LoRaWAN终端设备的所有相关信息。
 * 该模块处理设备的注册、状态跟踪、会话管理和数据存储等功能，是连接物理LoRaWAN设备与应用层的
 * 关键桥梁。
 * 
 * 设备模块支持多种设备类型（Class A/B/C）和激活方式（OTAA/ABP），并提供了设备会话状态的持久化
 * 存储，确保即使在服务器重启后也能保持设备的通信状态。此外，该模块还实现了帧计数器验证、消息
 * 完整性检查等安全机制，保障LoRaWAN网络的安全性。
 *
 * 文件功能
 * ========
 * 本文件(device.rs)实现了设备相关的数据库操作，提供了以下主要功能：
 * 1. 设备的CRUD（创建、读取、更新、删除）操作
 * 2. 设备会话状态的管理和持久化
 * 3. 设备上行消息的验证和处理
 * 4. 设备列表查询和过滤
 * 5. 设备状态统计（活跃/非活跃）
 ...
 * 主要组件
 * ========
 * - ValidationStatus枚举: 表示设备上行消息验证的结果状态
 * - DeviceClass枚举: 表示LoRaWAN设备类型（A/B/C）
 * - Device结构体: 表示设备的基本信息和状态
 * - DeviceChangeset结构体: 用于部分更新设备信息
 * - DeviceListItem结构体: 用于列表展示的设备信息
 * - Filters结构体: 用于设备列表查询的过滤条件
 * - create(): 创建新设备
 * - get(): 获取设备信息
 * - get_for_phypayload_and_incr_f_cnt_up(): 验证上行消息并更新帧计数器
 * - update(): 更新设备信息
 * - delete(): 删除设备
 * - get_active_inactive(): 获取活跃/非活跃设备统计
 * - get_data_rates(): 获取设备数据速率统计
 *
 * 关键流程
 * ========
 * 1. 设备注册流程:
 *    - 创建设备记录，关联应用和设备配置文件
 *    - 初始化设备会话（对于ABP设备）或准备OTAA激活
 *
 * 2. 设备上行消息处理流程:
 *    - 接收物理载荷(PhyPayload)
 *    - 验证消息完整性码(MIC)
 ...
 * 注意事项
 * ========
 * - 安全性: 严格验证上行消息，防止重放攻击和伪造
 * - 性能优化: 设备查询是高频操作，需要优化查询性能
 * - 会话管理: 正确处理设备会话状态，确保通信连续性
 * - 资源限制: 遵循租户设置的设备数量限制
 * - 数据一致性: 设备删除时需要清理相关资源和会话
 * - 设备类型: 不同类型设备(A/B/C)有不同的通信模式和处理逻辑
 */
