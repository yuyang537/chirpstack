 ...
 * 
 * # 模块概述
 * 本模块实现了ChirpStack系统的全局搜索功能，允许用户在整个系统中搜索各种实体，
 * 包括租户、应用、设备和网关。搜索支持基于文本和标签的过滤，并根据用户权限限制结果。
 * 
 * # 文件功能
 * - 提供全局搜索API，支持PostgreSQL和SQLite数据库
 * - 实现基于文本相似度的搜索算法
 * - 支持基于标签(tag)的过滤功能
 * - 根据用户权限过滤搜索结果
 * - 返回结构化的搜索结果，包含实体类型和相关信息
 * 
 * # 主要组件
 * - SearchResult：搜索结果的结构体，包含实体类型和详细信息
 * - global_search：全局搜索函数，支持PostgreSQL和SQLite
 * - parse_search_query：解析搜索查询，分离文本和标签过滤条件
 * 
 * # 关键流程
 * - 解析用户输入的搜索查询，提取文本和标签过滤条件
 * - 构建SQL查询，包含租户、应用、设备和网关的联合查询
 * - 根据用户权限过滤结果
 * - 计算相似度分数并排序结果
 * 
 * # 重要考虑事项
 * - 搜索结果受用户权限限制，全局管理员可以搜索所有实体
 * - PostgreSQL版本支持基于相似度的排序，SQLite版本不支持
 * - 标签搜索使用JSON/JSONB功能，实现方式在不同数据库中有所不同
 * - 搜索查询支持特殊语法：`key:value`用于标签过滤
 */
