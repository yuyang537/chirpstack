/**
 * @module schema_postgres
 * 
 * @description
 * 
 * # 模块概述
 * 本文件定义了ChirpStack系统在PostgreSQL数据库中的表结构和关系。这是由Diesel CLI自动生成的架构定义，
 * 用于支持Diesel ORM与PostgreSQL数据库的交互。
 * 
 * # 文件功能
 * - 定义所有数据库表的结构，包括字段名称、类型和约束
 * - 声明表之间的关系和外键约束
 * - 配置表的连接规则，支持复杂查询
 * 
 * # 主要组件
 ...
 * - 表之间的关系通过joinable!宏定义，确保正确的外键关联
 */
