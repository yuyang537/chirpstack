 ...
 * 模块概述
 * ========
 * 该模块是ChirpStack LoRaWAN网络服务器的漫游功能实现，支持LoRaWAN设备在不同网络间的漫游。
 * 在LoRaWAN生态系统中，漫游允许设备在离开其主网络覆盖范围时，通过访客网络继续通信。
 * 该模块实现了LoRaWAN后端接口规范中定义的漫游协议，包括被动漫游和转发漫游功能。
 *
 * 文件功能
 * ========
 * 该文件实现了与其他LoRaWAN网络服务器进行漫游通信的客户端功能。
 * 它管理漫游客户端连接池，处理漫游消息的转换，并提供判断设备是否为漫游设备的功能。
 * 主要功能包括初始化漫游客户端、根据NetID查找适当的客户端，以及在不同数据格式间进行转换。
 *
 * 主要组件
 * ========
 * - CLIENTS: 存储漫游客户端的全局连接池，按NetID索引
 * - setup(): 初始化漫游客户端连接池
 * - get(): 根据NetID查找适当的漫游客户端
 * - is_roaming_dev_addr(): 判断设备地址是否属于漫游设备
 * - 数据转换函数: 在ChirpStack内部格式和LoRaWAN后端接口格式之间转换
 *
 * 关键流程
 * ========
 * 1. 初始化漫游客户端，建立与配置的漫游服务器的连接
 * 2. 根据设备地址判断设备是否为漫游设备
 * 3. 对于漫游设备，查找适当的漫游客户端处理通信
 * 4. 在ChirpStack内部数据格式和LoRaWAN后端接口格式之间进行转换
 * 5. 处理漫游会话的生命周期管理
 *
 * 注意事项
 * ========
 * - 漫游功能依赖于正确配置的网络标识符(NetID)
 * - 被动漫游会话有生命周期限制，需要正确管理
 * - 密钥管理对于安全漫游至关重要，特别是在验证MIC时
 * - 数据格式转换必须精确，以确保不同系统间的互操作性
 * - 在生产环境中，应监控漫游连接状态和性能
 */
