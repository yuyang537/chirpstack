/**
 * @module helpers/tls22
 * 
 * @description
 * 
 * # 模块概述
 * 本模块提供了基于rustls 0.22版本的TLS（传输层安全）相关辅助功能。它是tls模块的一个变体，
 * 专门用于支持需要rustls 0.22版本的依赖项，如rumqttc。这种分离设计允许系统同时支持不同版本
 * 的rustls库，以满足不同组件的需求。
 * 
 * # 文件功能
 * - 加载和管理TLS根证书
 ...
 *   1. 读取证书文件内容
 *   2. 解析PEM格式的证书
 *   3. 转换为rustls 0.22可用的证书格式
 * 
 * - 私钥处理流程：
 *   1. 读取私钥文件内容
 *   2. 检测私钥格式（RSA、EC或PKCS#8）
 *   3. 必要时将私钥转换为PKCS#8格式
 *   4. 转换为rustls 0.22可用的私钥格式
 * 
 * # 重要考虑事项
 * - 与tls模块功能相似，但针对rustls 0.22版本
 * - 需要注意rustls版本兼容性问题
