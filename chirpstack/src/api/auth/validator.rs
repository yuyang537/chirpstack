 ...
 * 文件功能
 * ========
 * 本文件(validator.rs)实现了ChirpStack的授权验证系统，负责检查已认证用户对特定资源的访问权限。
 * 它定义了一系列验证器结构体，每个结构体负责验证对特定资源类型（如租户、应用程序、设备等）的访问权限。
 * 文件实现了基于角色的访问控制(RBAC)机制，根据用户角色和资源所有权确定访问权限。
 *
 * 主要组件
 * ========
 * - Flag: 表示资源操作类型的枚举（创建、读取、更新、删除、列表等）
 * - RequestValidator: 请求验证器，协调验证过程
 * - Validator: 验证器特性，定义了所有验证器必须实现的方法
 * - 各种具体验证器结构体：
 *   - ValidateActiveUser: 验证用户是否处于活动状态
 *   - ValidateIsAdmin: 验证用户是否为系统管理员
 *   - ValidateTenantAccess: 验证对租户的访问权限
 *   - ValidateApplicationAccess: 验证对应用程序的访问权限
 *   - ValidateDeviceAccess: 验证对设备的访问权限
 *   - 等等...
 *
 * 关键流程
 * ========
 * 1. API请求处理器接收到请求后，创建适当的验证器实例
 * 2. 调用RequestValidator.validate()方法，传入请求扩展和验证器
 * 3. 根据AuthID类型（用户或API密钥），调用相应的validate_user()或validate_key()方法
 * 4. 验证器执行数据库查询，检查用户或API密钥是否有权限访问请求的资源
 * 5. 如果验证成功，请求继续处理；如果失败，返回未授权错误
 *
 * 注意事项
 * ========
 * - 验证逻辑高度依赖数据库查询，性能可能受到数据库性能的影响
 * - 系统管理员(is_admin=true)通常拥有对所有资源的完全访问权限
 * - API密钥的权限范围通常比用户账户更受限
 * - 验证失败会返回unauthenticated状态错误，但不会提供详细原因（出于安全考虑）
 * - 内部错误会记录到日志中，但不会暴露给客户端
 * - 该文件包含大量重复的验证逻辑模式，但这种设计提供了细粒度的权限控制
 * - 对于性能关键的应用，可能需要考虑缓存常用的权限检查结果
 */
