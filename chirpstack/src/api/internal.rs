 ...
 * 模块概述
 * ========
 * 内部(Internal)模块是ChirpStack LoRaWAN网络服务器API框架的核心组件，负责处理系统内部
 * 操作和管理功能。该模块实现了用户认证、全局搜索、API密钥管理、系统设置查询、数据流处理
 * 等关键功能，为ChirpStack提供了必要的内部服务支持。
 * 
 * 内部模块是ChirpStack系统的"胶水层"，它连接了用户界面和底层服务，提供了统一的接口
 * 来访问系统的各种功能。该模块处理跨越多个领域的操作，如认证、搜索和监控，这些操作
 * 不适合放在特定的领域模块中。
 * 
 * 作为gRPC服务实现，内部模块提供了标准化的接口，使客户端应用程序能够以一致的方式
 * 访问ChirpStack的内部功能，同时确保适当的访问控制和数据验证。
 *
 * 文件功能
 * ========
 * 本文件(internal.rs)实现了InternalService gRPC服务，提供了以下主要功能：
 * 1. 用户认证：登录、获取用户配置文件、OpenID Connect和OAuth2认证
 * 2. 全局搜索：跨多个实体类型的搜索功能
 * 3. API密钥管理：创建、删除和列出API密钥
 * 4. 系统设置：获取系统配置和版本信息
 * 5. 数据流处理：流式获取网关帧、设备帧和设备事件
 * 6. 摘要信息：获取设备和网关的摘要信息
 * 7. 区域信息：列出和获取区域配置
 *
 * 主要组件
 * ========
 * - Internal结构体: 实现InternalService trait的主要结构
 * - login(): 用户登录
 * - profile(): 获取用户配置文件
 * - global_search(): 全局搜索
 * - create_api_key()/delete_api_key()/list_api_keys(): API密钥管理
 * - settings(): 获取系统设置
 * - open_id_connect_login()/o_auth2_login(): 外部认证集成
 * - get_devices_summary()/get_gateways_summary(): 获取摘要信息
 * - stream_gateway_frames()/stream_device_frames()/stream_device_events(): 流式数据处理
 * - list_regions()/get_region(): 区域配置管理
 * - get_version(): 获取系统版本
 *
 * 关键流程
 * ========
 ...
 * 注意事项
 * ========
 * - 安全考虑: 认证和API密钥管理需要特别注意安全性
 * - 性能影响: 全局搜索和数据流处理可能涉及大量数据
 * - 资源管理: 流式处理需要适当管理资源，避免泄漏
 * - 错误处理: 所有错误都应转换为适当的gRPC状态码
 * - 用户配置: 用户配置和系统设置应该安全存储和访问
 * - 外部依赖: 外部认证提供商的可用性会影响系统功能
 */
