 ...
 * 模块概述
 * ========
 * 网关(Gateway)模块是ChirpStack LoRaWAN网络服务器API框架的核心组件，负责处理与LoRaWAN网关
 * 相关的所有操作。该模块实现了网关的创建、查询、更新、删除等基本管理功能，以及网关指标查询、
 * 证书生成、中继网关管理等高级功能。
 * 
 * 网关是LoRaWAN网络架构中的关键基础设施，它负责接收终端设备的上行数据并将其转发到网络服务器，
 * 同时也负责将网络服务器的下行数据发送给终端设备。网关模块提供了完整的网关生命周期管理，
 * 确保网关能够正确地连接到ChirpStack并高效地转发数据。
 * 
 * 作为gRPC服务实现，网关模块提供了标准化的接口，使客户端应用程序能够以一致的方式管理
 * LoRaWAN网关，同时确保适当的访问控制和数据验证。
 *
 * 文件功能
 * ========
 * 本文件(gateway.rs)实现了GatewayService gRPC服务，提供了以下主要功能：
 * 1. 网关基本管理：创建、获取、更新、删除和列出网关
 * 2. 网关指标查询：获取网关性能指标和占空比指标
 * 3. 网关证书管理：生成网关客户端证书
 * 4. 中继网关管理：获取、更新、删除和列出中继网关
 *
 * 主要组件
 * ========
 * - Gateway结构体: 实现GatewayService trait的主要结构
 * - create(): 创建新网关
 * - get(): 获取网关详情
 * - update(): 更新网关配置
 * - delete(): 删除网关
 * - list(): 列出网关
 * - generate_client_certificate(): 生成网关客户端证书
 * - get_metrics(): 获取网关性能指标
 * - get_duty_cycle_metrics(): 获取网关占空比指标
 * - get_relay_gateway()/update_relay_gateway()/delete_relay_gateway()/list_relay_gateways(): 中继网关管理
 *
 * 关键流程
 * ========
 * 1. 网关创建流程:
 ...
 * - 性能影响: 网关指标查询可能涉及大量数据处理
 * - 证书安全: 网关证书生成需要确保密钥的安全性
 * - 错误处理: 所有错误都应转换为适当的gRPC状态码
 * - 中继功能: 中继网关功能需要特殊的网关和设备配置
 * - 占空比限制: 网关操作需要遵守区域性的占空比限制
 */
