 ...
 * 模块概述
 * ========
 * 多播组(Multicast Group)模块是ChirpStack LoRaWAN网络服务器API框架的重要组件，负责处理
 * LoRaWAN Class B/C多播功能相关的所有操作。该模块实现了多播组的创建、查询、更新、删除等
 * 基本管理功能，以及设备和网关的添加/移除、下行队列管理等高级功能。
 * 
 * 多播是LoRaWAN网络中的一项重要功能，它允许网络服务器同时向多个设备发送相同的下行消息，
 * 从而提高网络效率，特别适用于固件更新、批量命令等场景。多播组模块提供了完整的多播功能
 * 管理，包括多播组配置、成员管理和下行消息调度。
 * 
 * 作为gRPC服务实现，多播组模块提供了标准化的接口，使客户端应用程序能够以一致的方式管理
 * LoRaWAN多播功能，同时确保适当的访问控制和数据验证。
 *
 * 文件功能
 * ========
 * 本文件(multicast.rs)实现了MulticastGroupService gRPC服务，提供了以下主要功能：
 * 1. 多播组基本管理：创建、获取、更新、删除和列出多播组
 * 2. 多播组成员管理：添加/移除设备和网关
 * 3. 多播下行队列管理：入队下行消息、清空队列和列出队列项
 *
 * 主要组件
 * ========
 * - MulticastGroup结构体: 实现MulticastGroupService trait的主要结构
 * - create(): 创建新多播组
 * - get(): 获取多播组详情
 * - update(): 更新多播组配置
 * - delete(): 删除多播组
 * - list(): 列出多播组
 * - add_device()/remove_device(): 管理多播组设备成员
 * - add_gateway()/remove_gateway(): 管理多播组网关成员
 * - enqueue()/flush_queue()/list_queue(): 多播下行队列管理
 *
 * 关键流程
 * ========
 * 1. 多播组创建流程:
