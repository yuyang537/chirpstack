 ...
 * 模块概述
 * ========
 * 多播下行链路(Multicast)模块是ChirpStack LoRaWAN网络服务器下行链路处理框架的专用组件，负责实现
 * LoRaWAN多播通信功能。在LoRaWAN网络中，多播允许网络服务器向一组设备同时发送相同的下行数据，
 * 大大提高了批量通信的效率，特别适用于固件更新、群组命令等场景。
 * 
 * 该模块实现了LoRaWAN规范中定义的多播机制，包括多播组管理、多播下行队列处理和多播下行帧构建。
 * 它与设备管理、网关管理和队列管理模块紧密集成，为ChirpStack提供了高效的群组通信能力。
 * 
 * 在LoRaWAN网络中，多播通信主要用于Class B和Class C设备，这些设备能够在预定时间或持续接收下行数据。
 * 本模块通过优化网关选择和参数设置，确保多播消息能够被目标设备组高效接收，同时最小化网络资源消耗。
 *
 * 文件功能
 * ========
 * 本文件(multicast.rs)实现了多播下行链路的核心逻辑，提供了以下主要功能：
 * 1. 处理多播组队列项的调度
 * 2. 选择最佳网关集合用于多播发送
 * 3. 设置多播下行的发送参数
 * 4. 构建多播物理载荷
 ...
 * 主要组件
 * ========
 * - Multicast: 核心结构体，包含多播下行处理的所有状态和方法
 *   - handle_schedule_queue_item(): 处理多播队列项的调度
 *   - 各种内部方法用于设置TX信息、物理载荷等
 * 
 * - Node: 表示网络拓扑中的节点（网关或设备）的枚举
 * 
 * - 辅助函数:
 *   - enqueue(): 将多播消息入队到多播队列
 *   - get_minimum_gateway_set(): 计算覆盖所有目标设备的最小网关集合
 *   - get_gateway_set(): 获取可用的网关集合
 *   - get_gateway_device_count_map(): 计算每个网关覆盖的设备数量
 *
 * 关键流程
 * ========
 * 1. 多播队列调度流程:
 ...
 * 3. 最小网关集合计算流程:
 *    - 构建设备-网关连接图
 *    - 使用最小生成树算法计算覆盖所有设备的最小网关集合
 *    - 返回优化后的网关列表，用于多播发送
 *
 * 注意事项
 * ========
 * - 设备兼容性: 多播主要适用于Class B和Class C设备，Class A设备不支持多播
 * - 安全性: 多播使用共享密钥，安全级别低于单播，应谨慎使用
 * - 网关选择: 需要选择能够覆盖所有目标设备的最小网关集合，优化资源使用
 * - 参数设置: 多播参数需要考虑所有目标设备的能力，通常使用较低数据速率确保可靠性
 * - 队列管理: 多播队列需要适当管理，避免过度拥塞
 * - 资源限制: 多播可能占用大量网络资源，需要合理规划和限制
 * - 确认机制: 多播通常不支持确认，需要考虑消息可靠性的替代方案
 */
